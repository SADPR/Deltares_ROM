...
class PipingAnalysis():
    ...
    def __init__(self, simulation_type, parameters, GetAnalysisStageClass, CustomizeSimulation, mu_head):
        self.GetAnalysisStageClass = GetAnalysisStageClass
    ...
    def HeadUpdate(self, this_head):

        parameters = self.parameters.Clone()

        if "Left" in self.parameters['processes']['constraints_process_list'][0]['Parameters']['model_part_name'].GetString():
            parameters['processes']['constraints_process_list'][0]['Parameters']['reference_coordinate'].SetDouble(this_head)
        else:
            parameters['processes']['constraints_process_list'][1]['Parameters']['reference_coordinate'].SetDouble(this_head)

        return parameters



    def model_kratos_run(self, head):
        print(self.parameters['processes']['constraints_process_list'][1]['Parameters']['reference_coordinate'].GetDouble())
        parameters = self.HeadUpdate(head)
        print(parameters['processes']['constraints_process_list'][1]['Parameters']['reference_coordinate'].GetDouble())
        analysis_stage_class = self.GetAnalysisStageClass(parameters)
        model = KratosMultiphysics.Model()
        if self.simulation_type == 'FOM':
            simulation = self.CustomizeSimulation(analysis_stage_class,model,parameters, 'FOM' )
        elif self.simulation_type == 'ROM':
            analysis_stage_class = type(SetUpSimulationInstance(model, parameters))
            simulation = self.CustomizeSimulation(analysis_stage_class,model,parameters, 'ROM' )
        else:
            error
        simulation.Run()
        error_indicator = simulation.IsErroIndicatorAcceptable()
        residual_norm = simulation.GetResidualNorm()
        for process in simulation._GetListOfOutputProcesses():
            if isinstance(process, CalculateRomBasisOutputProcess):
                BasisOutputProcess = process
        SnapshotsMatrix = BasisOutputProcess._GetSnapshotsMatrix()
        pipe_active = self.get_pipe_active_in_elements(simulation)
        length = self.get_pipe_length(simulation)
        return all(pipe_active), length, SnapshotsMatrix, error_indicator, residual_norm
    ...
    def linear_search(self):
        ...
        pipe_length_total = []
        model = None
        while counter_head < len(self.mu_head):
            # check if pipe elements become active
            pipe_active, pipe_length, SnapshotsMatrix,error_indicator,residual_norm = self.model_kratos_run(self.mu_head[counter_head][0])
            pipe_length_total.append(pipe_length)
            self.solutions.append(SnapshotsMatrix)
            self.residual_norms.append(residual_norm)
            if self.simulation_type == 'FOM':

                if pipe_active:
                    # These are the values that a typical client will want to know (not the model outcome)
                    if counter_head > 0:
                        #self.solutions.pop()
                        return self.mu_head[counter_head - 1], pipe_length_total[counter_head - 1], self.residual_norms[counter_head - 1], self.mu_head[counter_head-1][0]
                    else:
                        raise Exception("ERROR: First head value attemped was high enough to produce full pipe. Try a range with a smaller lower bound")
                else:
                    # Record (and overwrite) any model output you need here.
                    # Thus it records the model at the critical head and length
                    pass
            elif self.simulation_type == 'ROM':

                if not error_indicator or pipe_active:
                    # These are the values that a typical client will want to know (not the model outcome)
                    if counter_head > 0:
                        #self.solutions.pop()
                        ...
                        # plt.plot(self.residual_norms, 'ro')
                        # plt.show()

                        return self.mu_head[counter_head - 1], pipe_length_total[counter_head - 1],  self.residual_norms[counter_head - 1], self.mu_head[counter_head-1][0]
                    else:
                        raise Exception("ERROR: First head value attemped produced a ROM failing to recover the FOM solution. Try increasing the training parameters")
                else:
                    # Record (and overwrite) any model output you need here.
                    # Thus it records the model at the critical head and length
                    pass
    ...
    def Run(self, search_type='linear'): # used to be called "critical_head_loop"
        critical_head_found = np.nan
        length = np.nan
        if search_type == 'linear':
           self.critical_head_found, self.length, self.residual_norm, self.critical_head = self.linear_search()
        else:
            raise Exception("ERROR: only linear search type implemented")


    def GetFinalData(self):
        return {f"pipe_length": np.array(self.length), "residual_norm":  np.array(self.residual_norm), "critical_head":  self.critical_head }
...
class RomManager(object):
    ...
    def FitPiping(self, mu_train=[None],mu_validation=[None], min_max_step = [None]):
        ...
        if chosen_projection_strategy == "galerkin":
            if type_of_decoder =="ann_enhanced":
                ...
                if any(item == "ROM" for item in training_stages):
                    self._LaunchTrainROM_Piping(mu_train,min_max_step)
                    self._ChangeRomFlags(simulation_to_run = "GalerkinROM")
                    self._LaunchROM_Piping(mu_train, min_max_step)
                if any(item == "HROM" for item in training_stages):
                    error
    ...
    def TestPiping(self, mu_test=[None], mu_train=[None], min_max_step = [None]):
        ...
        if chosen_projection_strategy == "galerkin":
            if type_of_decoder =="ann_enhanced":
                ...
                if any(item == "ROM" for item in testing_stages):
                    self._LoadSolutionBasis(mu_train)
                    self._LaunchFOM_Piping(mu_test, gid_and_vtk_name='FOM_Test', min_max_step = min_max_step)
                    self._ChangeRomFlags(simulation_to_run = "GalerkinROM")
                    self._LaunchROM_Piping(mu_test, gid_and_vtk_name='ROM_Test', min_max_step = min_max_step)
                if any(item == "HROM" for item in testing_stages):
                    error
    ...
    def Test(self, mu_test=[None], mu_train=[None]):
        ...
        #self.ComputeErrors(mu_test, 'Test') TODO not doing it as of now, but got to do it later
    ...
    def _LaunchTrainROM_Piping(self, mu_train, min_max_step):
        self._LaunchFOM_Piping(mu_train, min_max_step)
        self._LaunchComputeSolutionBasis(mu_train)


    def _LaunchFOM_Piping(self, mu_train, min_max_step, gid_and_vtk_name='FOM_Fit'):
        ...
        Hmax = min_max_step[1]
        step = min_max_step[2]
        #TODO accept specific ranges for each sample
        mu_head = list(drange(Hmin, Hmax, step))
        ...
        for Id, mu in enumerate(mu_train):
            ...
            if not fom_in_database:# or (NonConvergedSolutionsGathering and not nonconverged_fom_in_database):
                ...
                simulation = PipingAnalysis('FOM', parameters_copy,self._GetAnalysisStageClass, self.CustomizeSimulation, mu_head )
                simulation.Run()
    ...
    def _LaunchROM_Piping(self, mu_train, min_max_step, gid_and_vtk_name='ROM_Fit', nn_rom_interface = None):
        with open(self.project_parameters_name,'r') as parameter_file:
            parameters = KratosMultiphysics.Parameters(parameter_file.read())
        ...
        for Id, mu in enumerate(mu_train):
            ...
            if not in_database:
                ...
                self.UpdateMaterialParametersFile(materials_file_name, mu)
                #model = KratosMultiphysics.Model()
                simulation = PipingAnalysis('ROM', parameters_copy,self._GetAnalysisStageClass, self.CustomizeSimulation, mu_head )
                simulation.Run()
